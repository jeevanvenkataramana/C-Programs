        ...
        while (i < length)
        {
            ...
            if(!isalpha(line[i]))
            {
               ...
               if(i!=0 && (isalpha(line[i-1])))
               {
                  word_count++;
                  Word *a, *b;
                  a = tv->timed_insert(text);
                  b = tm->timed_insert(text);
               }
               text = "";
            }
            else
            ...
            if(i == length-1)
            {
               if(text.length()!=0)
               {
                  ...
                  word_count++;
                  Word *c, *d;
                  c = tv->timed_insert(text);
                  d = tm->timed_insert(text);
               }
    ...
    while (itv != vdata.end())
    {
        ...
        if (*itv != itm->second)
        {
           throw string(string("Data mismatch: ")
                    + itv->get_word() + ":" + to_string(itv->get_count())
                    + " and " + itm->second.get_word() + ":"
...
void time_word_searches(WordVector& v, WordMap& m) throw(string)
{
    ...
    for (int i = 1; i <= MAX_SEARCHES; i++)
    {
        ...
        int index = rand()%size;
        string text = vdata[index].get_word();

       // / Complete this function. /

        Word *a = tv->timed_search(text);
        Word *b = tm->timed_search(text);


         if(*a != *b) throw string("Search mismatch between vector and map");
    }
